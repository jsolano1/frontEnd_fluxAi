...
const Hero = () => {
  ...
  useEffect(() => {
    // El código para crear orbes y líneas permanece igual
    if (chaosRef.current) {
        const container = chaosRef.current;
        if (container.children.length > 0) return; // Evita duplicar elementos
        for (let i = 0; i < 12; i++) {
    ...
    <section className="min-h-screen flex items-center justify-center relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 text-center relative z-10">
        ...
        >
            ...
            <motion.p 
              className="text-lg text-accent font-semibold tracking-wider"
              variants={itemVariants}
            >
